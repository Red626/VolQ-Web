  ...
  $.fn.extend({
    flexslider: function(options) {
      ...
      if (options.animation.toLowerCase() == "slide") {
        ...
        slides.width(slider.width()/2).css({"float": "left"});
        container.width((slides.length * slider.width())); //extra width to account for quirks
        ...
        if (!ANIMATING) {
          ANIMATING = true;
          if (options.animation.toLowerCase() == "slide") {
      	    container.animate({"marginLeft": (-1 * target)* slider.width()/2 + "px"}, options.animationDuration, function(){
      	      ANIMATING = false;
      	      currentSlide = target;
      	    });
        	} else { //Default to Fade
      	...
      	$('.flex-direction-nav li a').click(function(event) {
      	  ...
      	  if (ANIMATING) {
        	  ...
        	  if ($(this).hasClass('next')) {
        	    var target = (slides.length>1) ? slides.length-2 : 0;
        	  } else {
        	    var target = 0;
        	  }
      ...
      if (options.slideshow && slides.length>2) {
        var animatedSlides;
      ...
      if (options.animation.toLowerCase() == "slide") {
        ...
        $(window).resize(function(){
          ...
          container.width((slides.length * slider.width()) + 20); //extra width to account for quirks
